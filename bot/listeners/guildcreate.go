...
func sendIntroMessage(ctx context.Context, worker *worker.Context, guild guild.Guild, userId uint64) {
	...
	msg := embed.NewEmbed().
		...
		AddField("Setup", "You can setup the bot using `/setup`, or you can use the [dashboard](https://ticket.labnat.se) which has additional options", false).
		AddField("Reaction Panels", fmt.Sprintf("Reaction panels are a commonly used feature of the bot. You can read about them [here](https://ticketsbot.net/panels), or create one on [the dashboard](https://ticket.labnat.se/manage/%d/panels)", guild.Id), false).
